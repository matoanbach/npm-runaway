...
import { useEffect, useState } from "react"
import { Progress } from "@/components/ui/progress"
import { TicketPercent, Clover } from "lucide-react"
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from "@/components/ui/tooltip"
...
export default function GanttChartComponent({ batches }: { batches: Batch[] }) {
  ...
  return (
    <div className="rounded-xl border bg-card text-card-foreground shadow overflow-hidden">
      ...
      {/* Main Content: Left Column for Batches, Right Column for Selected Batch's Expiry Details */}
      <div className="flex">
        {/* Left Column: Batch List */}
        <div className="w-40 border-r border-gray-200 dark:border-gray-800 p-4 space-y-4">
          {batches.map((batch) => (
        ...
        {/* Right Column: Table with Expiry Details for Selected Batch */}
        <div className="flex-1 p-6">
          {selectedBatch ? (

                <Table>
                  <TableCaption>
                    Expiry tracking for batch: {selectedBatch.label}
                  </TableCaption>
                  <TableHeader>
                    <TableRow>
                  ...
                  <TableBody>
                    {selectedBatch.products.map((product) => (
                      <TableRow key={product.name}>
                        <TableCell className="hidden sm:table-cell">{product.expiryDescription}</TableCell>
                        <TableCell className="hidden sm:table-cell">{product.dateRange}</TableCell>
                        <TableCell className="hidden sm:table-cell">{product.duration}</TableCell>
                        <TableCell>
                          <div className="flex items-center gap-2">
                            <Progress
                              value={product.progress}
                              className={`flex-1 ${
                                product.progress >= 80 ? "bg-red-400" : "bg-green-200"
                              }`}
                            />
                  ...
                  <TableFooter>
                    <TableRow>
                      <TableCell colSpan={5}>
                        Total Products: {selectedBatch.products.length}
                      </TableCell>
                    </TableRow>
                  </TableFooter>
                </Table>
